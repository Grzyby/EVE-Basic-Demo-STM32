...
void Cmd_Toggle(int16_t x, int16_t y, int16_t w, int16_t font, uint16_t options, uint16_t state, const char *str)
{
    ...
    Send_CMD(CMD_TOGGLE);
    Send_CMD((((uint32_t)y << 16) | (x & 0xffff)));
    Send_CMD((((uint32_t)font << 16) | (w & 0xffff)));
...
void Cmd_RomFont(int font, int romslot)
{
